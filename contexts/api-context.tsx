...
type ApiAction =
  ...
  | { type: "DELETE_REQUEST_FROM_COLLECTION"; payload: { collectionId: string; requestId: string } }
  ...
  | { type: "UPDATE_REQUEST_IN_COLLECTION"; payload: { collectionId: string; request: RequestConfig } }
...
function generateUniqueCollectionName(name: string, existingCollections: Collection[]): string {
...
function apiReducer(state: ApiState, action: ApiAction): ApiState {
  switch (action.type) {
    ...
    case "SET_CURRENT_REQUEST":
      return { ...state, currentRequest: action.payload, isRequestModified: false }
    ...
    case "ADD_REQUEST_TO_COLLECTION":
      const newRequest = { ...action.payload.request, collectionId: action.payload.collectionId }
      return {
        ...state,
        collections: state.collections.map((c) =>
          c.id === action.payload.collectionId
            ? { ...c, requests: [...c.requests, newRequest], updatedAt: new Date().toISOString() }
            : c,
        ),
        currentRequest: state.currentRequest?.id === newRequest.id ? newRequest : state.currentRequest,
        isRequestModified: false,
      }

    case "UPDATE_REQUEST_IN_COLLECTION":
      const updatedRequest = { ...action.payload.request, collectionId: action.payload.collectionId }
      return {
        ...state,
        collections: state.collections.map((c) =>
          c.id === action.payload.collectionId
            ? {
                ...c,
                requests: c.requests.map((r) => (r.id === action.payload.request.id ? updatedRequest : r)),
                updatedAt: new Date().toISOString(),
              }
            : c,
        ),
        currentRequest: state.currentRequest?.id === updatedRequest.id ? updatedRequest : state.currentRequest,
        isRequestModified: false,
      }
    ...
    case "ADD_TO_HISTORY":
      ...
      if (historyItem.length > settings.maxHistoryItems) {
        return { ...state, history: historyItem.slice(0, settings.maxHistoryItems) }
      }

      return { ...state, history: historyItem }
    ...
    case "LOAD_DATA":
      const { collections, history, environments, replaceExisting } = action.payload
      ...
      if (replaceExisting) {
        ...
        newCollections = collections.map((col) => ({
          ...col,
          requests: col.requests.map((req) => ({ ...req, collectionId: col.id })),
        }))
      } else {
        ...
        collections.forEach((newCol) => {
          const uniqueName = generateUniqueCollectionName(newCol.name, mergedCollections)
      ...
      return {
        ...
        history: replaceExisting ? history : [...state.history, ...history],
        environments: replaceExisting ? environments : [...state.environments, ...environments],
...
const ApiContext = createContext<{
  ...
  updateRequestInCollection: (collectionId: string, request: RequestConfig) => void
...
export function ApiProvider({ children }: { children: React.ReactNode }) {
  ...
  const findRequestCollection = (requestId: string): Collection | null => {
    return (
      state.collections.find((collection) => collection.requests.some((request) => request.id === requestId)) || null
    )
  }

  const getRequestResponse = (requestId: string): ApiResponse | null => {
    const historyItem = state.history.find((item) => item.request.id === requestId)
    return historyItem?.response || null
  }

  const saveRequestToCollection = (collectionId: string, request: RequestConfig) => {
    // Check if request already exists in collection
    const collection = state.collections.find((c) => c.id === collectionId)
    const existingRequest = collection?.requests.find((r) => r.id === request.id)

    if (existingRequest) {
      // Update existing request
      dispatch({ type: "UPDATE_REQUEST_IN_COLLECTION", payload: { collectionId, request } })
    } else {
      // Add new request
      dispatch({ type: "ADD_REQUEST_TO_COLLECTION", payload: { collectionId, request } })
    }
  }

  const updateRequestInCollection = (collectionId: string, request: RequestConfig) => {
    dispatch({ type: "UPDATE_REQUEST_IN_COLLECTION", payload: { collectionId, request } })
  }

  const deleteRequestFromCollection = (collectionId: string, requestId: string) => {
    dispatch({ type: "DELETE_REQUEST_FROM_COLLECTION", payload: { collectionId, requestId } })
  }
  ...
  const selectRequestWithUnsavedCheck = async (request: RequestConfig): Promise<boolean> => {
    if (state.isRequestModified) {
      return new Promise((resolve) => {
        const result = window.confirm("You have unsaved changes. Do you want to discard them and continue?")
        if (result) {
